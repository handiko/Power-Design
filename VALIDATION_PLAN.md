# VALIDATION PLAN — EC200U Telematics Tracker Power Subsystem
...
| T8 | Power-up sequencing | Oscilloscope (multi-channel: VIN, +3V3, +3V8, reset line, modem enable) | Cold power-up from 0 V | +3V3 reaches regulation and reset releases before modem enable asserts; modem enable never asserts while +3V3 is below the supervisor threshold |
...
- Test points on +3V3, +3V8, +5V, and the eFuse fault-flag line, accessible without disassembly, to allow production screening without requiring a full functional test rig for every unit.
